...
const char *DataSource::regions[] = {"AFRICA", "AMERICA", "ASIA", "EUROPE", "MIDDLE EAST"};

vector<const char *> DataSource::cLastParts = {
...
vector<const char *> DataSource::tpchAuxiliaries = {
        "do", "may", "might", "shall",
...
string DataSource::tpchSentence()
{
    if (randomTrue(1 / 5))
        ...
        return tpchNounPhrase() + " " + tpchVerbPhrase() + " " +
               tpchPrepositionalPhrase() + " " + tpchTerminators[randomUniformInt(0, tpchTerminators.size() - 1)];
    else if (randomTrue(1 / 5))
        return tpchNounPhrase() + " " + tpchVerbPhrase() + " " +
               tpchNounPhrase() + " " + tpchTerminators[randomUniformInt(0, tpchTerminators.size() - 1)];
    else if (randomTrue(1 / 5))
...
string DataSource::tpchNounPhrase()
{
    if (randomTrue(1 / 4))
        ...
        return string(tpchAdjectives[randomUniformInt(0, tpchAdjectives.size() - 1)]) + " " +
        ...
        return string(tpchAdverbs[randomUniformInt(0, tpchAdverbs.size() - 1)]) + " " +
               tpchAdjectives[randomUniformInt(0, tpchAdjectives.size() - 1)] + " " +
               tpchNouns[randomUniformInt(0, tpchNouns.size() - 1)];
}
...
string DataSource::tpchVerbPhrase()
{
    if (randomTrue(1 / 4))
        ...
        return string(tpchAuxiliaries[randomUniformInt(0, tpchAuxiliaries.size() - 1)]) + " " +
...
{
    return string(tpchPrepositions[randomUniformInt(0, tpchPrepositions.size() - 1)]) + " the " + tpchNounPhrase();
}
...
void DataSource::randomDouble(double minValue, double maxValue, int decimals, double &ret)
{
...
void DataSource::addAlphanumeric62(int length, ofstream &stream, bool delimiter)
{
    string s = "";
    int rand;
    for (int i = 0; i < length; i++) {
...
void DataSource::addAlphanumeric64(int length, ofstream &stream, bool delimiter)
{
    string s = "";
    int rand;
    for (int i = 0; i < length; i++) {
        rand = 0;
        while (rand == 0 || (rand > '9' && rand < 63) || (rand > 'Z' && rand < 'a'))
            randomUniformInt('0', 'z', rand);
        s += rand;
    }
    stream << s;
...
void DataSource::addAlphanumeric64(int minLength, int maxLength, ofstream &stream, bool delimiter)
{
    addAlphanumeric64(randomUniformInt(minLength, maxLength), stream, delimiter);
}

void DataSource::addAlphanumeric64Original(int minLength, int maxLength, ofstream &stream, bool delimiter)
{
    int rLength = 0;
...
void DataSource::addTextString(int minLength, int maxLength, ofstream &stream, bool delimiter)
{
...
void
DataSource::addTextStringCustomer(int minLength, int maxLength, const char *action, ofstream &stream, bool delimiter)
{
    int rLength = 0;
    randomUniformInt(minLength, maxLength, rLength);
    int l1 = 0;
...
void DataSource::addInt(int minValue, int maxValue, ofstream &stream, bool delimiter)
{
...
void DataSource::addDouble(double minValue, double maxValue, int decimals, ofstream &stream, bool delimiter)
{
    double d = 0;
...
Nation DataSource::getNation(int i)
{
    return nations[i];
}

const char *DataSource::getRegion(int i)
{
    return regions[i];
}
