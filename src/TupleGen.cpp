...
void TupleGen::genCustomer(int &cId, int &dId, int &wId, string &customerTime)
{
    ...
    customerStream << customerTime
                   << Config::getCsvDelim();                                        //C_SINCE - date/time given by the os when the CUSTOMER table was populated
...
void TupleGen::genOrder(int &oId, int &dId, int &wId, int &cId, int &olCount, string &orderTime)
{
    ...
    orderStream << orderTime
                << Config::getCsvDelim();                                                //O_ENTRY_D - current date/ time given by the os
    if (oId <=
        2100)                                                                                    //O_CARRIER_ID - random within [1 .. 10] if O_ID <= 2100, null otherwise
...
void TupleGen::genOrderline(int &oId, int &dId, int &wId, int &olNumber, string &orderTime)
{
    ...
    orderlineStream << (oId <= 2100 ? orderTime : "")
                    << Config::getCsvDelim();                            //OL_DELIVERY_D = O_ENTRY_D if OL_O_ID <= 2100, null otherwise
    orderlineStream << "5" << Config::getCsvDelim();                                                //OL_QUANTITY
    if (oId <=
        2100)                                                                                    //OL_AMOUNT = 0.00 if OL_O_ID <= 2100, random within [0.01..9999.99] otherwise
...
void TupleGen::genSupplier(int &suId)
{
    ...
    if ((suId + 7) % 1893 ==
        0)    //no spec																//SU_COMMENT: 5 rows "Customer%Recommends" + 5 rows "Customer%Complaints"
