...
bool DbcTools::connect(SQLHENV &hEnv, SQLHDBC &hDBC) {
  ...
  if (reviewReturn(hDBC, SQL_HANDLE_DBC, ret, 1)) {
	ret = SQLConnect(hDBC, (SQLCHAR *)Config::getDataSourceName().c_str(), SQL_NTS,
					 (SQLCHAR *)Config::getDbsUser().c_str(), SQL_NTS,
...
bool DbcTools::autoCommitOff(SQLHDBC &hDBC) {
  SQLRETURN ret = SQLSetConnectAttr(hDBC, SQL_ATTR_AUTOCOMMIT, SQL_AUTOCOMMIT_OFF, SQL_NTS);
  if (reviewReturn(hDBC, SQL_HANDLE_DBC, ret, 1))
	return 1;
...
bool DbcTools::bind(SQLHSTMT &hStmt, int pos, int &value) {
  SQLRETURN ret = SQLBindParameter(hStmt, pos, SQL_PARAM_INPUT, SQL_C_DEFAULT, SQL_INTEGER, 0, 0, &value, 0, 0);
  if (reviewReturn(hStmt, SQL_HANDLE_STMT, ret))
	return 1;
...
bool DbcTools::bind(SQLHSTMT &hStmt, int pos, double &value) {
  SQLRETURN ret = SQLBindParameter(hStmt, pos, SQL_PARAM_INPUT, SQL_C_DOUBLE, SQL_DOUBLE, 0, 0, &value, 0, 0);
  if (reviewReturn(hStmt, SQL_HANDLE_STMT, ret))
	return 1;
...
bool DbcTools::bind(SQLHSTMT &hStmt, int pos, int bufferLength, char *buffer) {
  SQLRETURN ret = SQLBindParameter(hStmt, pos, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_CHAR, bufferLength, 0, buffer, 0, 0);
  if (reviewReturn(hStmt, SQL_HANDLE_STMT, ret))
	return 1;
...
bool DbcTools::bind(SQLHSTMT &hStmt, int pos, SQL_TIMESTAMP_STRUCT &ts) {
  SQLRETURN ret = SQLBindParameter(hStmt, pos, SQL_PARAM_INPUT, SQL_C_TYPE_TIMESTAMP, SQL_TIMESTAMP, 0, 0, &ts, 0, 0);
  if (reviewReturn(hStmt, SQL_HANDLE_STMT, ret))
	return 1;
...
bool DbcTools::reviewReturn(SQLHANDLE &handle, SQLSMALLINT handleType, SQLRETURN &ret, bool showError) {
...
bool DbcTools::fetch(SQLHSTMT &hStmt, SQLCHAR *buf, SQLLEN *nIdicator, int pos, string &value) {
...
bool DbcTools::fetch(SQLHSTMT &hStmt, SQLCHAR *buf, SQLLEN *nIdicator, int pos, int &value) {
  if (fetch(hStmt, buf, nIdicator, pos)) {
...
bool DbcTools::fetch(SQLHSTMT &hStmt, SQLCHAR *buf, SQLLEN *nIdicator, int pos, double &value) {
  if (fetch(hStmt, buf, nIdicator, pos)) {
	value = atof((char *)buf);
	return 1;
  }
